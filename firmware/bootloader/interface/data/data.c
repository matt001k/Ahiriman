...
BL_STATIC BL_BOOL dReady[NUM_STATES] = {BL_FALSE};
...
{
    return Serial_RegisterCb(length_Cb);
}

BL_Err_t Data_LengthCbDeinit(void)
{
    return Serial_DeregisterCb();
}

BL_Err_t Data_DataCbInit(void)
{
    return Serial_RegisterCb(data_Cb);
}
